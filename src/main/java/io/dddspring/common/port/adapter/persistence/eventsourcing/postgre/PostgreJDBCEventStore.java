...
import io.dddspring.common.port.adapter.persistence.eventsourcing.DefaultEventStream;
...
public class PostgreJDBCEventStore implements EventStore, ApplicationContextAware {

    private static PostgreJDBCEventStore instance;

    private DataSource collaborationDataSource;
    private EventNotifiable eventNotifiable;
    private EventSerializer serializer;

    public synchronized static PostgreJDBCEventStore instance() {
        return instance;
    }

    public PostgreJDBCEventStore(DataSource aDataSource) {
        super();
    ...
    public List<DispatchableDomainEvent> eventsSince(long aLastReceivedEvent) {
        ...
        try {
            PreparedStatement statement =
                    connection
                        .prepareStatement(
                                "SELECT event_id, event_body, event_type FROM common.wl_es_event_store "
                                + "WHERE event_id > ? "
                                + "ORDER BY event_id");
    ...
    public EventStream eventStreamSince(EventStreamId anIdentity) {
        ...
        try {
            PreparedStatement statement =
                    connection
                        .prepareStatement(
                                "SELECT stream_version, event_type, event_body FROM common.wl_es_event_store "
    ...
    public void purge() {
        ...
        try {
            connection.createStatement().execute("delete from common.wl_es_event_store");
    ...
    throws Exception {
        ...
        PreparedStatement statement =
                aConnection
                    .prepareStatement(
                            "INSERT INTO common.wl_es_event_store (event_body,event_type,stream_name,stream_version) " +
//                                    "VALUES(:event_body, :event_type, :stream_name, :stream_version)")
        "VALUES(?, ?, ?, ?)");

//        statement.setLong(1, null);
        statement.setString(1, this.serializer().serialize(aDomainEvent));
